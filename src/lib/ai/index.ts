...
const client = new OpenAI({ baseURL: "https://api.model.box/v1", apiKey });
...
export async function textPrompt(
  ...
  const data = await client.chat.completions.create(
    { model: "deepseek/deepseek-chat", messages, ...body },
    { timeout: 30 * 1000 },
  );

  const msg = data.choices[0].message?.content;
  if (!msg) throw new Error("No response from AI", { cause: data });
  return msg;
}
